...
//!!!!!!!!!!!NOTICE!!!!!!!!!!!!!!
// len_in_bytes = 64  //len_in_bytes = 48??
// b_in_bytes = 32
// ell = 2    // ell = 3??
...
*/
  ...
  //!!!!!!!!!!NOTICE!!!!!!!!!!!!!!!
  // 5. len_in_bytes = 64 //len_in_bytes = 48 * 2 = 96??
...
*/
  ...
  let b3 = input_fourth_hash.sha256();
...
pub fn hash_to_field(data: Vec<u8>) -> [FieldElement; 2] {
  ...
  */
  ...
  //!!!!!!!!!!NOTICE!!!!!!!!!!!!!!
  // we need to extract L = 48 bytes = b1(32byte) || b2(first half bytes)
  // and then get fe_b1 from that 48 byte of data
  //similarly for fe_b2, remaining bytes of b2 || b3

  let (b10, b11, b12, b13) = decompose(b1);
  let (b20, b21, b22, b23) = decompose(b2);
  let (b30, b31, b32, b33) = decompose(b2);


  let fe_b1 = from_b256([b10, b11, b12, b13, b20, b21]);
  let fe_b2 = from_b256([b22, b23, b30, b31, b32, b33]);
  [fe_b1, fe_b2]
}