...
pub fn from_okm(data: [u8; 48]) -> FieldElement {
    ...
    let F_2_192: FieldElement = FieldElement {
        ls: [18446744069414584319, 18446744073709551614, 8589934592, 3],
    };

    // 192 bits per field element
    // ls[0] + ls[1] * 2^64 + ls[2] * 2^128 + ls[3] * 2^192
    let mut d0 = FieldElement {
        ls: [data_u64[2], data_u64[1], data_u64[0], 0],
    }.fe_to_montgomery();
    let mut d1 = FieldElement {
        ls: [data_u64[5], data_u64[4], data_u64[3], 0],
    }.fe_to_montgomery();
